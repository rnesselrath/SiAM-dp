...
public class LayoutStateFeature extends AbstractLayoutFeature implements
		ILayoutFeature {
	...
	public boolean layout(ILayoutContext context) {
		...
		int rectangleWidth = containerGa.getWidth() - AddStateFeature.INVISIBLE_RECT_RIGHT;
		...
		if (isParentDiagram) {
			...
			if (containerGa.getWidth() < MIN_WIDTH) {
				rectangleWidth = MIN_WIDTH - 2
						- AddStateFeature.INVISIBLE_RECT_RIGHT;
				containerGa.setWidth(MIN_WIDTH);
				anythingChanged = true;
			}
		...
		if (initialVisible) {
			rectangleWidth = containerGa.getWidth()
					- AddStateFeature.INVISIBLE_RECT_RIGHT
					- AddStateFeature.INVISIBLE_RECT_LEFT;
		}
		...
		while (iter.hasNext()) {
			...
			if (rectangleWidth != size.getWidth()) {
				if (graphicsAlgorithm instanceof Polyline) {
					...
					text.setWidth(rectangleWidth - 20);
					if (initialVisible) {
			...
			if (rectangleHeight != size.getHeight()) {
				if (graphicsAlgorithm instanceof Ellipse) {
					...
					ellipse.setWidth(20);
					ellipse.setHeight(20);
					ellipse.setY((rectangleHeight / 2) - 8);
					ellipse.setX(0);
				}
			}
