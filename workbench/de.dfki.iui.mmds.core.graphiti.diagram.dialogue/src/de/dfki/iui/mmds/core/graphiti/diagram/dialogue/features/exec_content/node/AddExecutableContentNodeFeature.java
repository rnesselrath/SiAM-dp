...
/**
 * Adding {@link ExecutableContentNode} feature
 * This class implements a pictogram element that has to be added. 
 * This pictogram element has to link to a {@link ExecutableContentNode} object.
 * 
 * @author Dragan
 *
 */
public class AddExecutableContentNodeFeature extends AbstractAddFeature implements IAddFeature {
	...
	/**
	 * Creates new {@link AddExecutableContentNodeFeature}
	 * @param fp the feature provider
	 */
	public AddExecutableContentNodeFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canAdd(IAddContext context) {
		...
		if (context.getNewObject() instanceof ExecutableContentNode) {
	...
	public PictogramElement add(IAddContext context) {
		...
		final ExecutableContentNode addedExecContentNode = (ExecutableContentNode) context.getNewObject();
		...
		if (context.getTargetContainer() instanceof Diagram) {
			...
			if (bo instanceof Dialogue && targetDiagram.getLink().getBusinessObjects().size() == 1) {
				...
				//
				//				Object[] bos = { addedExecContentNode, (Dialogue) bo };
				//				link(getDiagram(), bos);
		...
		// targetDiagram.getGraphicsAlgorithm();
		ContainerShape targetContainer = context.getTargetContainer();
		// GraphicsAlgorithm parentShape =
		// targetContainer.getGraphicsAlgorithm();

		//If the target container is of a type Sequence, find free holder to put the object
		...
		// Set a property that this shape is a part of the StateChart
		// package
		...
		RoundedRectangle rectangle; // need to access it later
		{
			...
			link(containerShape, addedExecContentNode);
		}
		...
		// SHAPE WITH TEXT NAME
		{
			...
			final Text text = gaService.createPlainText(shape, addedExecContentNode.getDisplayName());
			...
			// create link and wire it
			link(shape, addedExecContentNode);
		...
		{
			final Shape shape = peCreateService.createShape(containerShape, false);
			final Image image = gaService.createImage(shape, DialogueImageProvider.IMG_EXEC_CONTENT_NODE);
		...
		{
			final ContainerShape execHolderShape = peCreateService.createContainerShape(containerShape, true);
			// create and set graphics algorithm
			final Rectangle rec1 = gaService.createRectangle(execHolderShape);
			rec1.setStyle(StyleUtil.getStyleForSequence(getDiagram()));
			rec1.setFilled(false);
			gaService.setLocationAndSize(rec1, 0, 20, width, 26);
			PropertyUtil.setExecContentHolder(execHolderShape, PropertyUtil.EXEC_CONTENT_HOLDER_VALUE);
		...
		// It is not supported for objects from the Flow Chart, but in case
		// it does in future just uncomment it.
