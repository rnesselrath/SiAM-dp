...
/**
 * Add {@link Decision} Feature
 * 
 * @author Dragan Milchevski
 *
 */
public class AddDecisionFeature extends AbstractAddFeature implements IAddFeature{
	...
	/**
	 * Creates new {@link AddDecisionFeature}
	 * @param fp the feature provider
	 */
	public AddDecisionFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public PictogramElement add(IAddContext context) {
		...
		final Decision addedDecision = (Decision)context.getNewObject();
		...
		if(context.getTargetContainer() instanceof Diagram){
			...
			if(bo instanceof Dialogue && targetDiagram.getLink().getBusinessObjects().size() == 1){
				...
				//
				//				Object[] bos = {addedIf, (Dialogue)bo};
				//				link(getDiagram(), bos);
		...
		Rectangle rectangle; // need to access it later
		{
			...
			link(containerShape, addedDecision);
		}
		...
		// SHAPE WITH TEXT NAME
		{
			...
			// create and set text graphics algorithm
			final Text text = gaService.createPlainText(shape, addedDecision.getDisplayName());
			text.setStyle(StyleUtil.getStyleForStateChartText(getDiagram()));
		...
		{
			final Shape shape = peCreateService.createShape(containerShape, false);
			final Image image = gaService.createImage(shape, DialogueImageProvider.IMG_IF);
		...
		// SHAPE WITH TEXT FOR THEN
		{
			...
			// create and set text graphics algorithm
			final Text text = gaService.createPlainText(shape, "Then:");
			text.setFilled(false);
			gaService.setLocationAndSize(text, 10, 22, 30, 19);

		}

		{//SHAPE FOR THEN RECTANGLE
			final Shape shape = peCreateService.createContainerShape(containerShape, true);
			PropertyUtil.setThenElse(shape, PropertyUtil.THEN_VALUE);

			final RoundedRectangle recIf = gaService.createPlainRoundedRectangle(shape, 5, 5);
			gaService.setLocationAndSize(recIf, 10, 40, width/2-13, height-90);
			recIf.setStyle(StyleUtil.getStyleForSequence(getDiagram()));

			link(shape, addedDecision);
		}

		// SHAPE WITH TEXT FOR ELSE
		{
			// create shape for text
			final Shape shape = peCreateService.createShape(containerShape, false);
			PropertyUtil.setTextFieldVaue(shape, PropertyUtil.ELSE_TEXT_VALUE);

			// create and set text graphics algorithm
			final Text text = gaService.createPlainText(shape, "Else:");
			text.setFilled(false);
			gaService.setLocationAndSize(text, width/2, 22, 30, 19);

		}

		{//SHAPE FOR ELSE RECTANGLE
			final Shape shape = peCreateService.createContainerShape(containerShape, true);
			PropertyUtil.setThenElse(shape, PropertyUtil.ELSE_VALUE);

			final RoundedRectangle recThen = gaService.createPlainRoundedRectangle(shape, 5, 5);
			gaService.setLocationAndSize(recThen, width/2, 40, width/2-13, height-90);
			recThen.setStyle(StyleUtil.getStyleForSequence(getDiagram()));
