...
/**
 * {@link WhilePropertySource} class capable of supplying properties for 
 * display by the standard property sheet page implementation (PropertySheetPage) for the objects of type {@link While}

 * @author Dragan Milchevski
 *
 */
public class WhilePropertySource implements IPropertySource{
	...
	/**
	 * Creates new {@link WhilePropertySource}
	 * @param sc CompositeState
	 */
	public WhilePropertySource(While doWhile){
		this.w = doWhile;
	}
	...
	public void setPropertyValue(final Object id, final Object value) {
		TransactionalEditingDomain domain = TransactionUtil.getEditingDomain(w);
		domain.getCommandStack().execute(new RecordingCommand(domain) {
			public void doExecute() {
				if (NAME.equals(id)) {
					w.setDisplayName((String)value);
				}else if(ID.equals(id)){
					w.setId((String)value);
				}else if(CONDITION.equals(id)){
					w.setCondition((String)value);
				}
			}
		});
