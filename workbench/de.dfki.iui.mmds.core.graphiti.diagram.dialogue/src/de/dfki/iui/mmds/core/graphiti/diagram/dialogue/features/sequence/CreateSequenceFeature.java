...
/**
 *  Class for creating model content of type {@link Sequence}. The appropriate pictogram content is added by the add feature.
 * 
 * @author Dragan Milchevski
 *
 */
public class CreateSequenceFeature extends AbstractCreateFeature implements ICreateFeature{

	/**
	 * Creates new {@link CreateSequenceFeature}
	 * @param fp the feature provider.
	 */
	public CreateSequenceFeature(IFeatureProvider fp) {
		super(fp, "Sequence", "Create new Sequence");
	}
	...
	/**
	 * Create new {@link Sequence}. First get the container shape of the target object
	 * and check if is of the allowed instances create the {@link Sequence} inside.
	 * 
	...
	public Object[] create(ICreateContext context) {
		final Sequence newSequence = FlowchartFactory.eINSTANCE.createSequence();
		...
		if(!bos.isEmpty()){
			for(EObject bo : bos){
				if(BoUtil.addEObjectToParent(bo, newSequence, container)){
					break;
				}
			}
		}
		...
		addGraphicalRepresentation(context, newSequence);

		// activate direct editing after object creation
		getFeatureProvider().getDirectEditingInfo().setActive(true);

		return new Object[] { newSequence };
	}

	/**
	 * Get the icon for the {@link Sequence} from the Image provider class.
	 * 
	 * @return image for UI representation
	 */
	@Override
	public String getCreateImageId() {
		return DialogueImageProvider.IMG_SEQUENCE;
	}
