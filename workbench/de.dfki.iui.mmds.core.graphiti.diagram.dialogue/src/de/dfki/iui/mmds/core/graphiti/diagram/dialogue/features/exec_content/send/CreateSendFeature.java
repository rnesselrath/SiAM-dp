package de.dfki.iui.mmds.core.graphiti.diagram.dialogue.features.exec_content.send;
...
/**
 * Class for creating model content of type {@link Send}. The appropriate pictogram content is added by the add feature.
 * 
 * @author Dragan Milchevski
 * Created on: Jun 16, 2013
 *
 */
public class CreateSendFeature extends AbstractCreateFeature implements ICreateFeature {

	/**
	 * Creates new {@link CreateSendFeature}
	 * @param fp the feature provider.
	 */
	public CreateSendFeature(IFeatureProvider fp) {
		super(fp, "Send", "Create new Send Object");
	}
	 ...
	 * @return
	 */
	private ContainerShape findTargetContainer(ContainerShape targetContainer){
		ContainerShape currContainer = null;
		if(!PropertyUtil.isOnEntry(targetContainer) && !PropertyUtil.isOnExit(targetContainer)){
			currContainer = targetContainer.getContainer();
			findTargetContainer(currContainer);
		}
	...
	/**
	 * Create new {@link Send}. First get the container shape of the target object
	 * and check if is of the allowed instances create the {@link Send} inside.
	 * 
	...
	public Object[] create(ICreateContext context) {
		...
		addGraphicalRepresentation(context, send);
	...
	/**
	 * Get the icon for the {@link Send} from the Image provider class.
	 * 
	 * @return image for UI representation
	 */
	@Override
	public String getCreateImageId() {
		return DialogueImageProvider.IMG_SEND;
	}
