...
/**
 * Adding {@link WaitForEvent} feature
 * This class implements a pictogram element that has to be added. 
 * This pictogram element has to link to a {@link WaitForEvent} object.
 * 
 * @author Dragan Milchevski
 *
 */
public class AddWaitForEventFeature extends AbstractAddFeature implements IAddFeature{
	...
	/**
	 * Creates new {@link AddWaitForEventFeature}
	 * @param fp the feature provider
	 */
	public AddWaitForEventFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canAdd(IAddContext context) {
		...
		if(context.getNewObject() instanceof WaitForEvent){
	...
	public PictogramElement add(IAddContext context) {
		...
		final WaitForEvent addedWaitForEvent = (WaitForEvent) context.getNewObject();
		...
		if (context.getTargetContainer() instanceof Diagram) {
			...
			if (bo instanceof Dialogue && targetDiagram.getLink().getBusinessObjects().size() == 1) {
				...
				//
				//				Object[] bos = { addedWaitForEvent, (Dialogue) bo };
				//				link(getDiagram(), bos);
		...
		if (!isParentDiagram) {
			...
			RoundedRectangle rectangle; // need to access it later
			{
				...
				link(containerShape, addedWaitForEvent);
			}
			...
			// SHAPE WITH TEXT NAME
			{
				...
				final Text text = gaService.createPlainText(shape, addedWaitForEvent.getDisplayName());
				text.setStyle(StyleUtil.getStyleForStateChartText(getDiagram()));
				gaService.setLocationAndSize(text, 1, 1, width - 2, 19);

				// create link and wire it
				link(shape, addedWaitForEvent);
			...
			{
				final Shape shape = peCreateService.createShape(containerShape, false);
				final Image image = gaService.createImage(shape, DialogueImageProvider.IMG_WAIT_FOR_EVENT);
			...
			{
				final ContainerShape condEventHolderShape = peCreateService.createContainerShape(containerShape, true);
				// create and set graphics algorithm
				final Rectangle rec1 = gaService.createRectangle(condEventHolderShape);
				...
				PropertyUtil.setCondEventHolder(condEventHolderShape, PropertyUtil.COND_EVENT_HOLDER_VALUE);
				// PropertyUtil.setCollapsed(entryShape, "true");
			}

			{//CREATE SHAPE FOR MORE VALUES TEXT
