...
public class JSBindings implements Bindings {
	 ...
	 *
	 * @param context
	 *            SCXML Context to use for script variables.
	 * @param bindings
	 *            Javascript engine bindings for Javascript variables.
	 *
	 * @throws IllegalArgumentException
	 *             Thrown if either <code>context</code> or
	 *             <code>bindings</code> is <code>null</code>.
	 *
	 */
	...
	/**
	 * Returns <code>true</code> if the wrapped Bindings delegate or SCXML
	 * context contains a variable identified by <code>key</code>.
	 *
	 */
	@Override
	...
	/**
	 * Returns a union of the wrapped Bindings entry set and the SCXML context
	 * entry set.
	 * <p>
	...
	/**
	 * Returns the combined size of the wrapped Bindings entry set and the SCXML
	 * context entry set.
	 * <p>
	 * NOTE: doesn't seem to be invoked ever so not sure if it works in context.
	 * Not thread-safe.
	 *
	 */
	@Override
	...
	/**
	 * Returns <code>true</code> if the wrapped Bindings delegate or SCXML
	 * context contains <code>value</code>.
	 * <p>
	 * NOTE: doesn't seem to be invoked ever so not sure if it works in context.
	 * Not thread-safe.
	 */
	...
	/**
	 * Returns a union of the wrapped Bindings value list and the SCXML context
	 * value list.
	 * <p>
	 * NOTE: doesn't seem to be invoked ever so not sure if it works in context.
	 * Not thread-safe.
	...
	/**
	 * Returns a <code>true</code> if both the Bindings delegate and the SCXML
	 * context maps are empty.
	 * <p>
	 * NOTE: doesn't seem to be invoked ever so not sure if it works in context.
	 * Not thread-safe.
	 */
	...
	/**
	 * Returns the value from the wrapped Bindings delegate or SCXML context
	 * contains identified by <code>key</code>.
	 *
	 */
	@Override
	 ...
	 * <li>Delegates to the wrapped {@link Bindings#put(String, Object)} if the
	 * {@link Bindings} contains the key (name), else</li>
	 ...
	 * SCXML context. Not entirely sure about this implementation but it follows
	 * the philosophy of using the Javascript Bindings as a child context of the
	 * SCXML context.
	 * <p>
	 * NOTE: doesn't seem to be invoked ever so not sure if it works in context.
	 * Not thread-safe.
	...
	/**
	 * Delegates to the wrapped Bindings <code>clear</code> method. Does not
	 * clear the SCXML context.
	 * <p>
	 * NOTE: doesn't seem to be invoked ever so not sure if it works in context.
	 * Not thread-safe.
	 */
	 ...
	 * Bindings. Does a heavyweight copy - and so far only invoked by the not
	 * used methods.
	 */
