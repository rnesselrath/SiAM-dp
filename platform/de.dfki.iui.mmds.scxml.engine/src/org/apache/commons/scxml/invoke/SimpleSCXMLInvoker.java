...
/**
 * A simple {@link Invoker} for SCXML documents. Invoked SCXML document may not contain external namespace elements, further
 * invokes etc.
 */
public class SimpleSCXMLInvoker implements Invoker, Serializable {
	 ...
	 */
	@Override
	public void invoke(final String source, final Map<String, Object> params) throws InvokerException {
		SCXML scxml = null;
		try {
		...
		// executor.registerInvokerClass("grounding", GroundingInvoker.class);
		try {
			executor.go();
		} catch (ModelException me) {
			throw new InvokerException(me.getMessage(), me.getCause());
