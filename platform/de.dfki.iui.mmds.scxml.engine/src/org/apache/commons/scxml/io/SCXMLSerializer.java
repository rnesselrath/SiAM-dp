...
public class SCXMLSerializer {
	...
	public static String serialize(final SCXML scxml) {
		StringBuffer b = new StringBuffer("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n").append("<scxml xmlns=\"").append(NAMESPACE_SCXML).append("\"").append(serializeNamespaceDeclarations(scxml))
				.append(" version=\"").append(scxml.getVersion()).append("\" initial=\"").append(scxml.getInitial()).append("\">\n");
		if (XFORMER == null) {
			org.apache.commons.logging.Log log = LogFactory.getLog(SCXMLSerializer.class);
			log.warn("SCXMLSerializer: DOM serialization pertinent to" + " the document will be skipped since a suitable" + " JAXP Transformer could not be instantiated.");
		}
	 ...
	 * 
	 * @param b
	 *            The buffer to append the serialization to
	 * @param s
	 *            The State to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeState(final StringBuffer b, final State s, final String indent) {
	 ...
	 *            The buffer to append the serialization to
	 * @param p
	 *            The Parallel to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeParallel(final StringBuffer b, final Parallel p, final String indent) {
	 ...
	 *            The buffer to append the serialization to
	 * @param i
	 *            The Invoke to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeInvoke(final StringBuffer b, final Invoke i, final String indent) {
		...
		for (Param p : params)
			b.append(indent).append(INDENT).append("<param name=\"").append(p.getName()).append("\" expr=\"").append(SCXMLHelper.escapeXML(p.getExpr())).append("\"/>\n");
	 ...
	 *            The buffer to append the serialization to
	 * @param i
	 *            The Initial to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeInitial(final StringBuffer b, final Initial i, final String indent) {
	 ...
	 *            The buffer to append the serialization to
	 * @param l
	 *            The List of History objects to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeHistory(final StringBuffer b, final List<History> l, final String indent) {
		if (l.size() > 0)
			for (History h : l) {
	 ...
	 *            The indent for this XML element
	 */
	public static void serializeTransition(final StringBuffer b, final Transition t, final String indent) {
		...
		if (!SCXMLHelper.isStringEmpty(t.getCond()))
			b.append(" cond=\"").append(SCXMLHelper.escapeXML(t.getCond())).append("\"");
		if (!SCXMLHelper.isStringEmpty(t.getNext()))
			b.append(" target=\"").append(SCXMLHelper.escapeXML(t.getNext())).append("\"");
		b.append(">\n");
	 ...
	 *            The indent for this XML element
	 */
	public static void serializeDatamodel(final StringBuffer b, final Datamodel dm, final String indent) {
		...
		if (data != null && data.size() > 0) {
			...
			for (Data datum : data) {
				...
				if (dataNode != null) {
					...
					b.append(indent).append(INDENT).append("<data id=\"").append(datum.getId()).append("\" expr=\"").append(SCXMLHelper.escapeXML(datum.getExpr())).append("\" />\n");
			}
			b.append(indent).append("</datamodel>\n");
		}
	}
	 ...
	 *            The buffer to append the serialization to
	 * @param t
	 *            The TransitionTarget whose OnEntry is to be serialized
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeOnEntry(final StringBuffer b, final TransitionTarget t, final String indent) {
	 ...
	 *            The buffer to append the serialization to
	 * @param t
	 *            The TransitionTarget whose OnExit is to be serialized
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeOnExit(final StringBuffer b, final TransitionTarget t, final String indent) {
	 ...
	 */
	public static boolean serializeActions(final StringBuffer b, final List<Action> l, final String indent) {
		if (l == null)
			return false;
		boolean exit = false;
		for (Action a : l)
			if (a instanceof Var) {
				Var v = (Var) a;
				b.append(indent).append("<cs:var name=\"").append(v.getName()).append("\" expr=\"").append(SCXMLHelper.escapeXML(v.getExpr())).append("\"/>\n");
				...
				if (!SCXMLHelper.isStringEmpty(asn.getLocation())) {
					...
					if (!SCXMLHelper.isStringEmpty(asn.getSrc()))
						...
						b.append("\" expr=\"").append(SCXMLHelper.escapeXML(asn.getExpr()));
				} else
					b.append(" name=\"").append(asn.getName()).append("\" expr=\"").append(SCXMLHelper.escapeXML(asn.getExpr()));
				b.append("\"/>\n");
				...
				Log lg = (Log) a;
				b.append(indent).append("<log expr=\"").append(SCXMLHelper.escapeXML(lg.getExpr())).append("\"/>\n");
				...
				ElseIf eif = (ElseIf) a;
				b.append(indent).append("<elseif cond=\"").append(SCXMLHelper.escapeXML(eif.getCond())).append("\" />\n");
			}
		return exit;
	}
	 ...
	 *            The buffer to append the serialization to
	 * @param send
	 *            The Send object to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeSend(final StringBuffer b, final Send send, final String indent) {
		b.append(indent).append("<send");
		if (send.getId() != null)
			b.append(" id=\"").append(send.getId()).append("\"");
	 ...
	 *            The model element containing the body content
	 * @return String The serialized body content
	 */
	public static final String getBodyContent(final ExternalContent externalContent) {
	 ...
	 *            The buffer to append the serialization to
	 * @param iff
	 *            The If object to serialize
	 * @param indent
	 *            The indent for this XML element
	 */
	public static void serializeIf(final StringBuffer b, final If iff, final String indent) {
		b.append(indent).append("<if cond=\"").append(SCXMLHelper.escapeXML(iff.getCond())).append("\">\n");
	 ...
	 */
	private static void serializeTransitionTargetAttributes(final StringBuffer b, final TransitionTarget t) {
		String id = t.getId();
		if (id != null)
	 ...
	 * 
	 * @param holder
	 *            The {@link NamespacePrefixesHolder} object
	 * @return The serialized namespace declarations
	 */
	private static String serializeNamespaceDeclarations(final NamespacePrefixesHolder holder) {
			...
			for (Map.Entry<String, String> entry : namespaces.entrySet()) {
				...
				if (prefix == null || prefix.length() == 0) {
					if (!nsURI.equals(NAMESPACE_SCXML)) {
						org.apache.commons.logging.Log log = LogFactory.getLog(SCXMLSerializer.class);
						log.warn("When using the SCXMLSerializer, the default " + "namespace must be the SCXML namespace:" + NAMESPACE_SCXML);
					}
				} else if (prefix.equals("cs") && !nsURI.equals(NAMESPACE_COMMONS_SCXML)) {
					org.apache.commons.logging.Log log = LogFactory.getLog(SCXMLSerializer.class);
					log.warn("When using the SCXMLSerializer, the namespace" + "prefix \"cs\" must bind to the Commons SCXML " + "namespace:" + NAMESPACE_COMMONS_SCXML);
				} else if (prefix.length() > 0)
					b.append(" xmlns:").append(prefix).append("=\"").append(nsURI).append("\"");
			}
		return b.toString();
	}
