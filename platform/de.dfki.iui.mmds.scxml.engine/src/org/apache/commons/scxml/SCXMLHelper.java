...
import org.apache.commons.scxml.model.IParentState;
...
public final class SCXMLHelper {
	...
	public static Map<String,TransitionTarget> getAllSubStates(final IParentState parent) {
		HashMap<String,TransitionTarget> result = new HashMap<String,TransitionTarget>();
		
		Map<String,TransitionTarget> tmp = parent.getSubStates();
		result.putAll(tmp);
		
		Collection<TransitionTarget> tts = tmp.values();
		for(TransitionTarget tt : tts) {
			if (tt instanceof IParentState) {
				IParentState p = (IParentState)tt;
				result.putAll(getAllSubStates(p));
			}
		}		
        ...
        if (p.isCrossRegion()) {
            for (State region : p.getRegionsExited()) {
                ...
                for (TransitionTarget tt : par.getChildren()) {
                    State s = (State) tt;
                    for (TransitionTarget a : currentStates) {
                    	//http://svn.apache.org/viewvc/commons/proper/scxml/branches/J6/src/main/java/org/apache/commons/scxml/SCXMLHelper.java?r1=730827&r2=1132823&diff_format=h
