...
/**
 * The class in this SCXML object model that corresponds to the &lt;if&gt; SCXML
 * element, which serves as a container for conditionally executed elements.
 * &lt;else&gt; and &lt;elseif&gt; can optionally appear within an &lt;if&gt; as
 * immediate children, and serve to partition the elements within an &lt;if&gt;.
 * 
 */
	...
	public void execute(final EventDispatcher evtDispatcher, final ErrorReporter errRep, final SCInstance scInstance, final Log appLog, final Collection<TriggerEvent> derivedEvents)
		...
		for (Action aa : actions) {
			if (execute && !(aa instanceof ElseIf) && !(aa instanceof Else)) {
				...
				ctx.setLocal(getNamespacesKey(), getNamespaces());
				execute = eval.evalCond(ctx, ((ElseIf) aa).getCond()).booleanValue();
				ctx.setLocal(getNamespacesKey(), null);
			}
		}
	}
