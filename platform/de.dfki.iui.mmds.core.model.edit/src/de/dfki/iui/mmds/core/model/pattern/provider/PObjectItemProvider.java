 ...
 * {@link de.dfki.iui.mmds.core.model.pattern.PObject} object. <!--
...
public class PObjectItemProvider extends PPatternItemProvider {
	...
	protected static final EClassItemProvider eclassProvider = (EClassItemProvider) ecoreAdapterFactory.createEClassAdapter();
	...
	public String getText(Object object) {
		EClass type = ((PObject) object).getType();
		String label = null;
		if (type != null) {
			label = type.getName();
		}
		String varName = ((PObject) object).getVarName();
		return (label == null || label.length() == 0 ? getString("_UI_PObject_type") : (label)) + (varName != null ? " - " + varName : "");
	}
	...
	protected void collectNewChildDescriptors(Collection<Object> newChildDescriptors, Object object) {
		...
		if (o.getType() != null) {
			...
			List<EStructuralFeature> features = new LinkedList<EStructuralFeature>(o.getType().getEAllStructuralFeatures());
			features.removeAll(usedFeatures);

			for (EStructuralFeature feature : features) {
				// don't show the entity_id as property
				if (feature.getFeatureID() == BasePackage.ENTITY__ENTITY_ID && ((EClass) feature.eContainer()).getInstanceClass() == Entity.class) {
					continue;
				}
